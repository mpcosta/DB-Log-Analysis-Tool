...
def get_most_popular_artc(x):
    ...
    c.execute("SELECT title, views FROM articles \
               ...
               FROM log GROUP BY path) AS popular_articles \
               ON path LIKE '%%' || slug \
               ORDER BY views DESC \
               LIMIT %s", (x,))
    ...
    if len(query_results) < int(x):
        print("\nUnfortunately this database \
                does not contain %s Articles." % (x,))
        x = len(query_results)
        print("\nInstead we will be showing you \
                all the %s Articles available.\n" % (x,))
...
def get_most_popular_authors():
    ...
    c.execute("SELECT name, sum(views) FROM authors, articles \
               LEFT JOIN (SELECT path, count(*) AS views \
               FROM log GROUP BY path) AS all_articles \
               ON path LIKE '%%' || slug \
...
def get_most_err_req(x):
    ...
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("SELECT notOKStatus.dates, \
               (notOKStatus.total*100.0/allStatus.total) AS percentColumn \
               FROM (SELECT CAST(time AS DATE) AS dates, \
               count(status) AS total FROM log \
               WHERE status NOT LIKE 200 || '%%' GROUP BY dates) \
               AS notOKStatus \
               JOIN (SELECT CAST(time AS DATE) AS dates, \
               count(status) AS total FROM log \
               GROUP BY dates) AS allStatus \
               ON notOKStatus.dates = allStatus.dates \
               WHERE (notOKStatus.total*100.0/allStatus.total) > %s", (x,))
    query_results = c.fetchall()
    # Check Query List Size and Compare with percentage Requested in x param
    if len(query_results) <= 0:
        print("\nUnfortunately we could not find any results \
                that led to more than %s Percent." % (x,))
        print("Pleasy try again with a different percentage value. \n")
    else:
        # Print out the days that more than X percent of requests lead to errs
        print("\nThe Dates in which more than %s %% of requests \
                led to errors are:\n" % (x,))
        for author, views in query_results:
